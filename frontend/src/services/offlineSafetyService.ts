...
export interface SafetyThresholds {
  cautionKm: number; // default: 15.0 km
  dangerKm: number;  // default: 5.0 km
}
...
export interface GeofenceEvaluation {
  ...
  state: SafetyState;
  level: SafetyLevel;
  nearestPoint: [number, number]; // [lat, lon] on boundary line
...
export function evaluateLocalGeofence(
  ...
  let closestProjLat = lat;
  let closestProjLon = lon;
  ...
  // Determine 3-tier safety level (SAFE -> CAUTION -> DANGER)
  const dangerLimit = thresholds?.dangerKm ?? THRESHOLD_WARNING_KM;
  const cautionLimit = thresholds?.cautionKm ?? THRESHOLD_APPROACHING_KM;

  let level: SafetyLevel = 'SAFE';
  if (roundedDistance <= dangerLimit || state === 'BREACH' || state === 'WARNING') {
    level = 'DANGER';
  } else if (roundedDistance <= cautionLimit || state === 'APPROACHING') {
    level = 'CAUTION';
  } else {
    level = 'SAFE';
  }
  ...
  return {
    ...
    level,
    nearestPoint: [
      Math.round(closestProjLat * 10000) / 10000,
      Math.round(closestProjLon * 10000) / 10000,
    ],
