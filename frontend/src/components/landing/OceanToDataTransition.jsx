...
export const OceanToDataTransition = React.memo(() => {
  return (
    <section className="relative min-h-[60vh] w-full flex flex-col justify-center px-6 sm:px-10 lg:px-16 xl:px-20 py-16 z-10 select-none pointer-events-none">
      ...
      <div className="w-full flex flex-col lg:flex-row justify-between items-start gap-8">
        ...
        <div className="w-full lg:w-[32%] max-w-md text-left space-y-3.5 pointer-events-auto">
          <div className="inline-flex items-center gap-2 text-xs font-mono text-cyan-700 dark:text-cyan-400 uppercase tracking-widest bg-cyan-100 dark:bg-cyan-950/50 px-3 py-1 rounded-full border border-cyan-300 dark:border-cyan-800/50 backdrop-blur-sm">
            <Satellite className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
            <span>Layer 01 — Satellite Ingestion</span>
          </div>

          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight leading-[1.15]">
          ...
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 font-light leading-relaxed">
            Every second, orbital satellites and coastal ocean buoys capture terabytes of thermal, optical, and gravitational data across the Indian Ocean Basin.
          </p>
        </div>
        ...
        <div className="w-full lg:w-[32%] max-w-md grid grid-cols-2 gap-3 pointer-events-auto">
          {[
            ...
            <div
              key={idx}
              className="p-3.5 rounded-2xl glass-panel-subtle hover:glass-panel border border-slate-200 dark:border-slate-800/80 hover:border-cyan-400/50 transition-all duration-300 group text-left"
            >
              ...
              <div className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100 font-mono group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors">
                {item.val}
              </div>
              <div className="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5">{item.label}</div>
            </div>
          ))}
        </div>
      </div>
