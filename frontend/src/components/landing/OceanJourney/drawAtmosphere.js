/**
 * drawAtmosphere — Renders the Top Sky, volumetric realistic clouds,
 * soft atmospheric horizon haze, storm sky darkening, lightning engine,
 * golden dawn recovery, and safe haven lighthouse within 0–75% scroll.
 */
import { scrollMap, scrollMapColor, getHorizonRatio, lerp } from './types.js';

// Dynamic Sky Top color palette across scroll journey (compressed into 0–75%):
const SKY_TOP_COLORS = [
  [0.00, '#e0f2fe'], // Bright pale sky-blue / white
  [0.14, '#94a3b8'], // Soft overcast slate
  [0.24, '#334155'], // Dark storm blue-grey
  [0.36, '#090d16'], // Deep storm charcoal
  [0.45, '#0f172a'], // High-altitude stratosphere
  [0.54, '#1e293b'], // Pre-dawn clear
  [0.64, '#fdba74'], // Golden sunrise break
  [0.72, '#bae6fd'], // Serene morning harbor sky
  [0.78, '#0f2942'], // Submerging transition
  [0.85, '#020b18'], // Deep marine
];

const SKY_HORIZON_COLORS = [
  [0.00, '#ffffff'], // Luminous pure white soft horizon
  [0.14, '#cbd5e1'], // Soft grey horizon haze
  [0.24, '#475569'], // Stormy horizon
  [0.36, '#0b1120'], // Thunderous sea horizon
  [0.45, '#1e293b'],
  [0.54, '#334155'],
  [0.64, '#fed7aa'], // Warm golden horizon
  [0.72, '#f0f9ff'], // Clean bright harbor horizon
  [0.78, '#0a2238'], // Submerging transition
  [0.85, '#020b18'],
];
...
export function drawAtmosphere(rc) {
  const { ctx, width, height, time, scroll, mouseX, mouseY, isMobile } = rc;
  const horizonRatio = getHorizonRatio(scroll);
  const horizonY = height * horizonRatio + mouseY * 12;

  // Sky visibility factor (fades out as ocean submerges past 0.75)
  const skyFade = scrollMap(scroll, [
    [0.00, 1.0],
    [0.75, 1.0],
    [0.82, 0.2],
    [0.87, 0.0],
  ...
  for (let i = 0; i < CLOUDS.length; i++) {
    const c = CLOUDS[i];
    if (isMobile && c.layer === 1 && i % 2 !== 0) continue;
    ...
    puffGrad.addColorStop(0, `rgba(${cloudR}, ${cloudG}, ${cloudB}, ${c.density * (0.8 + stormDarkness * 0.3) * skyFade})`);
    puffGrad.addColorStop(0.55, `rgba(${cloudShadeR}, ${cloudShadeG}, ${cloudShadeB}, ${c.density * 0.65 * skyFade})`);
  ...
  /* ── 3. Golden Dawn / Sunrise Break (Compressed to 0.56 - 0.72) ── */
  ...
  if (dawnGlow > 0.02) {
    ...
    dawnGrad.addColorStop(0, `rgba(255, 240, 180, ${0.75 * dawnGlow * skyFade})`);
    dawnGrad.addColorStop(0.3, `rgba(251, 146, 60, ${0.45 * dawnGlow * skyFade})`);
    dawnGrad.addColorStop(0.7, `rgba(217, 70, 239, ${0.15 * dawnGlow * skyFade})`);
    ...
    ctx.fillStyle = `rgba(255, 250, 220, ${0.95 * dawnGlow * skyFade})`;
  ...
  /* ── 4. Lightning Engine (Compressed to 0.28 - 0.48) ── */
  ...
  /* ── 5. Distant Safe Harbor & Lighthouse (Compressed to 0.58 - 0.75) ── */
  ...
  if (havenVisibility > 0.02) {
    ...
    ctx.fillStyle = `rgba(15, 35, 60, ${0.85 * havenVisibility * skyFade})`;
    ctx.beginPath();
    ...
    // Lighthouse Tower
    ctx.fillStyle = `rgba(30, 50, 80, ${havenVisibility * skyFade})`;
    ctx.beginPath();
    ...
    // Lantern Room
    ctx.fillStyle = `rgba(255, 235, 150, ${havenVisibility * skyFade})`;
    ...
    beamGrad.addColorStop(0, `rgba(255, 248, 190, ${0.8 * havenVisibility * skyFade})`);
    beamGrad.addColorStop(0.35, `rgba(255, 220, 130, ${0.35 * havenVisibility * skyFade})`);
  ...
  /* ── 6. Middle: Soft Atmospheric Horizon Haze ── */
  ctx.save();
  const hazeHeight = 70;
  ...
  hazeGrad.addColorStop(0.45, `rgba(224, 242, 254, ${0.75 * (1 - stormDarkness * 0.6) * skyFade})`);
  hazeGrad.addColorStop(0.7, `rgba(14, 116, 144, ${0.45 * (1 - stormDarkness * 0.5) * skyFade})`);
