...
export const ScrollJourney = ({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      /* ── Layer visibility computations compressed into 0–75% ── */
      const heroO = jp <= 0.04 ? 1 : jp >= 0.16 ? 0 : 1 - (jp - 0.04) / 0.12;
      const heroTY = -jp * 180;
      const satO = layerOpacity(jp, 0.08, 0.18, 0.30, 0.40);
      const dataO = layerOpacity(jp, 0.28, 0.38, 0.48, 0.58);
      const orcaO = layerOpacity(jp, 0.46, 0.56, 0.68, 0.75);
      const hintO = jp <= 0.01 ? 1 : jp >= 0.06 ? 0 : 1 - (jp - 0.01) / 0.05;

      if (heroRef.current) {
        heroRef.current.style.opacity = String(heroO);
        heroRef.current.style.visibility = heroO > 0.001 ? 'visible' : 'hidden';
        heroRef.current.style.transform = `translate3d(0,${heroTY}px,0)`;
      }
      if (satRef.current) {
        satRef.current.style.opacity = String(satO);
        satRef.current.style.visibility = satO > 0.001 ? 'visible' : 'hidden';
        satRef.current.style.transform = `translate3d(0,${(1 - satO) * 30}px,0)`;
      }
      if (dataRef.current) {
        dataRef.current.style.opacity = String(dataO);
        dataRef.current.style.visibility = dataO > 0.001 ? 'visible' : 'hidden';
        dataRef.current.style.transform = `translate3d(0,${(1 - dataO) * 25}px,0)`;
      }
      if (orcaRef.current) {
        orcaRef.current.style.opacity = String(orcaO);
        orcaRef.current.style.visibility = orcaO > 0.001 ? 'visible' : 'hidden';
      ...
      if (hintRef.current) {
        ...
        hintRef.current.style.visibility = hintO > 0.001 ? 'visible' : 'hidden';
      }
    };
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-950/60 border border-cyan-500/20 text-cyan-300 text-xs font-mono mb-8 backdrop-blur-md shadow-lg shadow-cyan-950/50">
            ...
            <span className="text-slate-300 font-medium">Marine Ecosystem Intelligence</span>
          </div>
        ...
        >
          <div className="relative z-10 text-center mb-4 px-4">
            ...
            <p className="text-sm text-slate-400 mt-2 max-w-lg mx-auto">
              Orbital satellites and coastal sensors scan terabytes of thermal,
